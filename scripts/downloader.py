...
try:
    ...
    JD_API_AVAILABLE = APP_KEY and APP_KEY != "your_app_key_here"
    if JD_API_AVAILABLE:
        print("[启动] 京东 API 模块加载成功，将优先使用 API 获取京东商品信息")
    else:
        print("[启动] 京东 API 未配置密钥，将使用爬虫模式")
except ImportError:
    JD_API_AVAILABLE = False
    print("[启动] 京东 API 模块未安装，将使用爬虫模式")
...
class ProductDownloader:
    def __init__(self, output_dir="downloads"):
        ...
        # 初始化京东 API 客户端
        self.jd_api_client = None
        if JD_API_AVAILABLE:
            try:
                self.jd_api_client = JDApiClient(APP_KEY, APP_SECRET)
            except Exception as e:
                print(f"  京东 API 客户端初始化失败: {e}，将使用爬虫模式")
    ...
    async def download_jd_via_api(self, url):
        """通过京东 API 获取商品信息（无需浏览器，速度快 10-20 倍）

        Returns:
            (title, cover_urls) 或 None（API 失败时）
        """
        if not self.jd_api_client:
            return None

        print(f"  [API 模式] 尝试通过京东 API 获取商品信息...")
        try:
            info = self.jd_api_client.get_product_info(url)

            # 检查是否获取到了有效数据
            if not info.get('title'):
                print(f"  [API 模式] 未获取到商品标题，回退到爬虫模式")
                return None

            if not info.get('cover_images'):
                print(f"  [API 模式] 未获取到头图，回退到爬虫模式")
                return None

            title = info['title']
            cover_urls = info['cover_images']

            print(f"  [API 模式] 成功！标题: {title[:50]}...")
            print(f"  [API 模式] 头图: {len(cover_urls)} 张")
            return title, cover_urls

        except Exception as e:
            print(f"  [API 模式] 失败: {e}，回退到爬虫模式")
            return None
    ...
    async def download_product(self, browser, url_or_keyword):
        ...
        try:
            ...
            if self.is_url(url_or_keyword):
                ...
                # 京东商品：优先使用 API 模式（无需打开浏览器）
                api_result = None
                if platform == "jd" and self.jd_api_client:
                    api_result = await self.download_jd_via_api(url)

                if api_result:
                    # API 成功，直接使用 API 返回的数据
                    title, cover_urls = api_result
                else:
                    # API 未启用或失败，使用爬虫模式
                    # 创建新页面并设置用户代理
                    ...
                    try:
                        # 根据平台调用对应的下载方法
                        if platform == "jd":
                            title, cover_urls = await self.download_jd(page, url)
                        elif platform == "tmall":
                            title, cover_urls = await self.download_tmall(page, url)
                        elif platform == "taobao":
                            title, cover_urls = await self.download_taobao(page, url)
                        else:
                ...
                try:
                    title, cover_urls = await self.search_taobao(page, keyword)
                finally:
                    await page.close()
